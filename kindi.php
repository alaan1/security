...
ob_start();
##########
$token = "5405536762:AAGBzcUIO8h9Xoa37-kAetUDSavlHbzjuvg"; // توكنك
$kindi = "5405536762:AAGBzcUIO8h9Xoa37-kAetUDSavlHbzjuvg";// توكنك

$buy = "M_L_F";
$userbot ="SRSH0BOT";
$sudo = "2067261869"; // ايديك
$admin = "$sudo";
$idBot = "5405536762";// ايدي بوتك
$admmm = $sudo;
$ADMIN = $sudo;
$Dev = array("$sudo","2067261869");// ايديك
$Devd = "$sudo";
$tbbots = 'SRSH0BOT';/// يوزر بوتك بدون @
 
$channel = "FoR4lEx";/// يوزر قناتك بدون@
$chinnl = "$channel";
$DevUser = "@$buy"; // 
$Devch = "@$buy"; //  
$Userbot = "$tbbots"; // 
...
$proxre = array("*⌯ حياتي اتفضل معاك العملاق*",
"*⌯ هلا عمري امرني انا العملاق*",
"*⌯ كول حياتي شرايد كساع و صحت بسمي*",
"*⌯ لاإله إلا الله وحده لاشريك له*");
$reprox = array_rand($proxre, 1);
if($text == "عملاق" and $namebot == NULL){
if ($tc == 'group' | $tc == 'supergroup'){
...
if($text == "الغاء قائمة التثبيت" or $text == "الغاء كل التثبيت"){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$carlos) or in_array($from_id,$nazar) or in_array($from_id,$eri)) { 
...
if ($text =="تفعيل الزخرفة" or $text == "تفعيل الزخرفه"){
if($status == "creator" || $status == "administrator" || in_array($from_id,$Dev) || in_array($from_id,$developer) || in_array($from_id,$carlos) || in_array($from_id,$eri) || in_array($from_id,$nazar)) {$add = $settings["information"]["added"];
if ($add == true) { 
 bot('sendmessage',[
 'chat_id'=>$chat_id,
...
elseif($text =="تعطيل الزخرفة" or $text == "تعطيل الزخرفه"){
if($status == "creator" || $status == "administrator" || in_array($from_id,$Dev) || in_array($from_id,$developer)|| in_array($from_id,$carlos) || in_array($from_id,$eri) || in_array($from_id,$nazar)) {$add = $settings["information"]["added"];
if ($add == true) {
 bot('sendmessage',[
 'chat_id'=>$chat_id,
...
$k = file_get_contents("https://aba.ae/en/ftplogin/nssar/apikindi/api.php?text=".$text);
bot('sendMessage',[
'chat_id'=>$chat_id,
'text'=>"$k ـ────────────
...
bot("sendmessage",['chat_id'=>$chat_id,
'text'=>"Welcome To Source NIZK\n    
✵⁞ [Nizk Team](https://t.me/TH3NK)
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
⌯︙[Source Channel](https://t.me/TH3NK)\n
⌯︙[Source Install](https://t.me/TH1BS)\n
⌯︙[Changing ID Channel](https://t.me/TH3NK)\n
⌯︙[TEAM Source](https://t.me/TH3NK)*\n
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
⌯︙[ConnectDeV](https://t.me/AIikindi)
",'parse_mode'=>"Markdown",
'disable_web_page_preview'=> true ,
'reply_to_message_id'=>$message_id,
'reply_markup'=>json_encode([
'inline_keyboard'=>[
[['text'=>"- تنصيب سورس",'url'=>"https://telegram.me/Snizkbot?start"]],
...
##########
if ($text == "⌯ تحديث" or $text == "تحديث" and in_array($from_id,$Dev)){
...
if ($text == "⌯ تحديث السورس" or $text == "تحديث السورس" and in_array($from_id,$Dev)){
 $ver = phpversion();  
